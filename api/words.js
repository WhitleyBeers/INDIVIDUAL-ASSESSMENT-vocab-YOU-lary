...
const getAbc = (uid) => new Promise((resolve, reject) => {
  fetch(`${endpoint}/vocab.json?orderBy="uid"&equalTo="${uid}"`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    ...
    .then((response) => response.json())
    .then((data) => {
      const abcCards = Object.values(data).sort((a, b) => a.title.localeCompare(b.title));
      resolve(abcCards);
    })
    .catch(reject);
});

const getOldest = (uid) => new Promise((resolve, reject) => {
  fetch(`${endpoint}/vocab.json?orderBy="uid"&equalTo="${uid}"`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    ...
    .then((response) => response.json())
    .then((data) => {
      const oldestCards = Object.values(data).sort((a, b) => a.time_submitted.localeCompare(b.time_submitted));
      resolve(oldestCards);
    })
    .catch(reject);
});

const getNewest = (uid) => new Promise((resolve, reject) => {
  fetch(`${endpoint}/vocab.json?orderBy="uid"&equalTo="${uid}"`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    ...
    .then((response) => response.json())
    .then((data) => {
      const newestCards = Object.values(data).sort((b, a) => a.time_submitted.localeCompare(b.time_submitted));
      resolve(newestCards);
    })
    .catch(reject);
});
...
export {
  getWords, createWord, updateWord, deleteWord, getSingleWord, getHtml, getCSS, getJS, getAbc, getOldest, getNewest
};
